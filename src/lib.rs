//! The goal of the functionality within this module is to find a common subdirectory `D` for a set
...
fn naive_find_common_dir<P: AsRef<Path>, I: IntoIterator<Item = P> + Clone>(
    paths: I,
) -> Result<CommonDir, Error> {
    let mut iter = paths.clone().into_iter();
    let first_path = iter.next().ok_or(Error::Empty)?;
    let first_path = first_path.as_ref();

    let path_trunk = first_path
        .parent()
        .ok_or_else(|| Error::NoRootDirectory(first_path.to_path_buf()))?;
...
pub enum Error {
    #[error("Empty input: got no path(s)")]
    Empty,

    #[error("Unable to mirror input directory to output: found an invalid file path: '{0}'")]
    InvalidPath(PathBuf),

    #[error("No root directory for path: '{0}'")]
    NoRootDirectory(PathBuf),
}

// The intention of this function is to remove a common root path from a given path.
fn unroot(root: &Path, path: &Path) -> PathBuf {
